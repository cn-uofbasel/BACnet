...
void loop(){
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
      ...
      delay(1000);                // Delay to give user time to acknowledge that tag has been read
...
void updateGpsData() { 
  if (gps.location.isValid() && gps.location.isUpdated()) // only update information if it is new
  {
    gpsFix = true;                    // Used to determine display information
    
    Serial.print("New longitude: ");
    longitude = gps.location.lng();   // Adjust global variable for longitude
    Serial.print(longitude);
    Serial.println("");

    Serial.print("New latitude: ");
    latitude = gps.location.lat();    // Adjust global variable for latitude
...
// Print coordinates if GPS fix has been found, else message stating it's still searching
void printGpsData() {
  if(!gpsFix){
    lcd.clear();
    lcd.setCursor(0,0);
    ...
    lcd.setCursor(0,0);
    lcd.print("Lng: ");
    lcd.print(longitude, 6); // six decimal places
    lcd.setCursor(0,1);
    lcd.print("Lat: ");
