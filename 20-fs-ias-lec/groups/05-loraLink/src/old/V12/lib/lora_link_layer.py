...
class Lora_Link_Layer:
    ...
    def lora_cb(self, lora):
        ...
        if events & LoRa.RX_PACKET_EVENT:
            ...
            [meta,data]=self.unpack_frame(msg)
            msg_utf8 = data.decode("utf-8")
            #msg_utf8 = msg.decode("utf-8")
            print('Link Layer: Lora packet received. ' + msg_utf8)
    ...
    def lora_send_csma_ca(self, msg, use_ca):
        if (use_ca):
            ...
            while not (rts_random_key == self.incoming_cts_key):
                #maximum repetition
                if not self.wait:
    ...
    def lora_send(self, msg):
        # handle type: 1 is controll 2 is data
        type=1
        frame2send=self.pack_frame(type,msg)
        print("frame packed:")
        print(frame2send)
        self.s.send(frame2send)
    ...
    def pack_frame(self, type, data):
        ...
        frame=type.to_bytes(1,'big')+self.lora_mac_address_b+data
        return frame


    def unpack_frame(self, frame):
        meta = [frame[0], frame[1:len(self.lora_mac_address_b)]]
        data = frame[len(self.lora_mac_address_b)+1:len(frame)]
        return meta, data
