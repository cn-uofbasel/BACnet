...
class HMAC:
    ...
    def __init__(self, key, msg=None, digestmod=''):
        ...
        if callable(digestmod):
            ...
            self.digest_cons = lambda d=b'': _hashlib.sha256(d) #Changed for LOPY4
        else:
...
def digest(key, msg, digest):
    ...
    if callable(digest):
        ...
        digest_cons = lambda d=b'': _hashlib.sha256(d) #Changed for LOPY4
    else:
    ...
    inner.update(key) #changed for LOPY4
    inner.update(msg)
    inner_digest = inner.digest()

    outer = digest_cons()
    #outer.update(key.translate(trans_5C))
    outer.update(key) #changed for LOPY4
    outer.update(inner_digest)
    return outer.digest()


def digest_backup(key, msg, digest):
    """Fast inline implementation of HMAC.
