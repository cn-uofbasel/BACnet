...
class Event:
    def __len__(self):
        return len(self.meta)+len(self.signature)+len(self.content)
    ...
    @classmethod
    def from_cbor(cls, event):  # Read in an Event from cbor format (parameter is bytes()), creates a new Event object
        meta, signature, content = cbor2.loads(event)
        print(meta, signature, content)
